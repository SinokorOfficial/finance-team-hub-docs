...
| `GET /api/export/status.xlsx`, `group-fill.xlsx` | ★ 과제만 담깁니다 |
| `GET /api/files/{fid}`, `POST /api/files/zip` | ★ 과제에 붙은 첨부만. 어느 과제에도 안 붙은 파일도 막습니다 |

화면 쪽은 같은 규칙(`visProj`)으로 한 번 더 걸러 내고, 통계 줄에서 `★`·`승인 대기` 버튼을 감추고
"조회 전용 계정 — 그룹 AX 대시보드 등록 대상(★) 과제 N건만 보입니다" 안내를 띄웁니다.
다만 실제 경계는 서버입니다 — 화면을 고쳐도 다른 과제는 내려오지 않습니다.
