...
- **과제 행(`.ai`)** — 왼쪽 막대(`.sq`)가 상태 색, 과제명을 누르면 상세 창. `url` 이 있으면 `[열기 ↗]` 로 새 탭. 개발 문서 사이트(`docsUrl`)는 상세 화면에서만 열립니다(v3.23).
...
목록의 행을 누르면 상세 창(`openModal`)이 열립니다. 위쪽에 등록자·등록일·담당자(계정)·최근 수정·승인 이력·그룹 과제번호·개발 문서(`docsUrl` 이 있으면 `[열기 ↗]`)가 `.mmeta` 로 나오고, 그 아래 입력 항목이 두 절로 갈립니다.

- **A · 그룹 대시보드(기획팀) 등록 항목** — 과제명, 담당자, 담당자 계정, 카테고리, 시작일/목표 완료일, 현재 업무 방식, AI 활용 목표, 기대효과, 효과 지표 블록, 사용안내 요약, 매뉴얼 문서 안내문·첨부파일, 링크, 개발 문서 사이트 안내문·주소, ★ 기획팀 전달 대상, 등록 상태 메모, 그룹 과제번호.
...
| 보완필요 | 승인된 과제인데 필수가 비었음 — 사용안내 요약·매뉴얼 문서 첨부(상태가 '운영중'일 때) 또는 효과 지표 |
...
**한 번에 내려받기** — 프로젝트 툴바의 `📎 첨부 내려받기` 를 누르면 창이 열리고, **지금 목록에 보이는 과제**(상태·업무 영역·담당자·검색 조건이 걸린 그대로)의 첨부파일이 과제 제목별로 묶여 나옵니다. 파일마다 체크박스가 있고 과제 제목 줄의 체크박스로 그 과제 전체를, 창 아래 `[전체 선택]` 으로 전부 고를 수 있습니다(일부만 고르면 과제 체크박스는 중간 표시). 고른 파일은 서버가 압축파일 하나로 묶어 내려주며, 안에서 **과제명 폴더로 나뉩니다**. 1개만 골랐을 때는 압축하지 않고 그 파일을 그대로 받습니다. 한 번에 300개·합계 200MB까지이고, 파일마다 있는 `따로 받기` 로 한 개씩 받을 수도 있습니다.
...
5. 조회 전용 계정에서는 등록·수정 버튼이 CSS 한 줄로 사라지고(`body[data-role="viewer"] .needs-edit`), 보이는 과제도 ★(그룹 대시보드 등록 대상)로 한정됩니다([RBAC 1.1절](rbac.md)).
