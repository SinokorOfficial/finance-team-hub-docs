...
| `manual` | 사용안내(매뉴얼) 요약 | 여러 줄 글(3~5줄) — 허브 상세 창에서 바로 읽는 요약 | 개발 상태가 `운영중`일 때만 ● |
| `url` | 링크(URL) | 한 줄 글. 주소 형식만 저장(앞에 `https://`를 자동으로 붙여 줍니다) | |
| `docsUrl` | 개발 문서 사이트(URL) | 선택. 그 과제의 개발 문서(MkDocs 공개 사이트) 주소. 편집 폼에 "개발 문서 사이트란?" 안내문이 함께 있고, 상세 창 머리(`.mmeta`)에서 `[열기 ↗]`로 열림 (v3.22, v3.23에서 목록·메인 카드 노출은 빼고 상세만 남김) | |
| `files` | 첨부파일(매뉴얼 문서) | 배열 `[{id, name, size, by, at}]` — 사내 서버판에서만 올릴 수 있습니다. 1건 이상이면 충족(빈 배열은 미첨부로 판정) | 개발 상태가 `운영중`일 때만 ● (v3.24, [ADR-0011](adr/0011-manual-document-attachment.md)) |
